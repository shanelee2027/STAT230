...
```{r}
wb = read.csv("WB_2024.csv")
dim(wb)
names(wb)
head(wb, 5)
sapply(wb, class)
# GNI has class integer
wb_Subset <- wb[wb$GNI > 70000 & is.na(wb$GNI)==F,c("Country", "GNI", "EnergyUse", "Measles")]
wb_Subset
...
```{r}
plot(wb$Rural, wb$Measles, xlab="Rural", ylab="Measles", main="Scatterplot of Rural vs Measles", col="Blue", pch='x')
